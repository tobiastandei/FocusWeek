(function () {
  ...
  function getDayData() { const idx=activeTab==='today'?getTodayIndex():currentWeekDay; if(!state.days[idx])state.days[idx]={foco:[],ops:[]}; return state.days[idx]; }
  ...
  function showLoginScreen() {
    ...
    if (!el) {
      ...
      el.innerHTML = '<div style="font-size:40px;margin-bottom:12px;">🎯</div><div style="font-size:24px;font-weight:500;margin-bottom:8px;">FocusWeek</div><div style="font-size:14px;color:var(--text-secondary);margin-bottom:32px;">Tu semana, organizada.</div><input id="login-email" type="email" placeholder="tu@email.com" style="width:100%;max-width:280px;padding:12px;font-size:14px;font-family:var(--font);border:0.5px solid var(--border-strong);border-radius:10px;background:var(--surface);color:var(--text);outline:none;margin-bottom:10px;display:block;margin-left:auto;margin-right:auto;"><button id="login-btn" style="padding:12px 28px;border:none;border-radius:10px;background:#534AB7;color:white;font-size:14px;font-family:var(--font);cursor:pointer;font-weight:500;">Entrar con email</button><div id="login-msg" style="margin-top:16px;font-size:13px;color:var(--text-tertiary);display:none;max-width:300px;margin-left:auto;margin-right:auto;"></div>';
    ...
    document.getElementById('login-btn').onclick = async () => {
      ...
      if (error) { msg.textContent = 'Error: ' + error.message; msg.style.display = 'block'; btn.disabled = false; btn.textContent = 'Entrar con email'; }
  ...
  function render() { if(activeTab==='today')renderToday(); if(activeTab==='week')renderWeek(); if(activeTab==='notes')renderNotes(); }
  ...
  function renderHitos() {
    ...
    list.innerHTML = '';
    (state.hitos||[]).forEach(h => { const li=document.createElement('li');li.className='hito';li.innerHTML='<span class="hito-emoji">'+(h.emoji||'🎯')+'</span><span>'+h.text+'</span>';list.appendChild(li); });
    if (!state.hitos.length) list.innerHTML = '<li class="hito" style="color:var(--text-tertiary);font-style:italic;">Sin hitos. Clickeá Editar para agregar.</li>';
  }
  ...
  function renderTaskList(group, tasks, listId, barId, pctId, color) {
    ...
    tasks.forEach((t,idx)=>{const ind=document.createElement('div');ind.className='drop-indicator';ind.dataset.group=group;ind.dataset.pos=String(idx);el.appendChild(ind);el.appendChild(makeTaskEl(t,idx,group));});
    const lastInd=document.createElement('div');lastInd.className='drop-indicator';lastInd.dataset.group=group;lastInd.dataset.pos=String(tasks.length);el.appendChild(lastInd);
  ...
  function makeTaskEl(t, idx, group) {
    ...
    const grip=document.createElement('div');grip.className='grip';grip.innerHTML='<div class="grip-row"><div class="grip-dot"></div><div class="grip-dot"></div></div>'.repeat(3);grip.draggable=true;
    ...
    if(isEdit){const inp=document.createElement('input');inp.className='edit-input';inp.id='ei-'+t.id;inp.value=t.text;inp.addEventListener('keydown',e=>{if(e.key==='Enter')commitEdit();if(e.key==='Escape')cancelEdit();});inp.addEventListener('click',e=>e.stopPropagation());content.appendChild(inp);setTimeout(()=>{inp.focus();inp.selectionStart=inp.value.length;},10);}
    else{const txt=document.createElement('div');txt.className='task-text';txt.textContent=t.text;content.appendChild(txt);content.addEventListener('click',e=>{if(e.target.closest('[data-addnote]'))return;e.stopPropagation();startEdit(t.id,group);});}
    if(isNoteEdit){const ni=document.createElement('input');ni.className='note-input';ni.id='ni-'+t.id;ni.value=t.note||'';ni.placeholder='Agregar nota...';ni.addEventListener('keydown',e=>{if(e.key==='Enter')commitNoteEdit();if(e.key==='Escape')cancelNoteEdit();});ni.addEventListener('click',e=>e.stopPropagation());content.appendChild(ni);setTimeout(()=>ni.focus(),10);}
    else if(t.note&&!t.done){const noteEl=document.createElement('div');noteEl.className='task-note';noteEl.dataset.addnote='1';noteEl.textContent=t.note;noteEl.addEventListener('click',e=>{e.stopPropagation();startNoteEdit(t.id,group);});content.appendChild(noteEl);}
    else if(!t.done&&!isEdit){const btn=document.createElement('button');btn.className='add-note-btn';btn.dataset.addnote='1';btn.textContent='+ nota';btn.addEventListener('click',e=>{e.stopPropagation();startNoteEdit(t.id,group);});content.appendChild(btn);}
    div.appendChild(content);
    const actions=document.createElement('div');actions.className='task-actions';
    if(isEdit||isNoteEdit)actions.style.opacity='1';
    const editBtn=document.createElement('button');editBtn.className='ib';editBtn.textContent='✏️';editBtn.addEventListener('click',e=>{e.stopPropagation();isEdit?commitEdit():startEdit(t.id,group);});
    const delBtn=document.createElement('button');delBtn.className='ib';delBtn.textContent='🗑️';
    delBtn.addEventListener('click',async e=>{e.stopPropagation();const saved=snap();const label=t.text;const day=getDayData();day[group].splice(idx,1);await deleteTask(t.id);render();showUndo(label,saved);});
  ...
  async function handleDrop(e) {
    ...
    await saveTask(item,finalGroup,dayIdx);render();
  }
  ...
  async function commitEdit(){
    ...
    if(inp&&inp.value.trim()){const day=getDayData();const task=day[editGroup].find(t=>t.id===editId);if(task){task.text=inp.value.trim();const dayIdx=activeTab==='today'?getTodayIndex():currentWeekDay;await saveTask(task,editGroup,dayIdx);}}
  ...
  async function commitNoteEdit(){
    ...
    if(inp){const day=getDayData();const task=day[editNoteGroup].find(t=>t.id===editNoteId);if(task){task.note=inp.value.trim();const dayIdx=activeTab==='today'?getTodayIndex():currentWeekDay;await saveTask(task,editNoteGroup,dayIdx);}}
  ...
  function renderWeek(){
    ...
    if(strip){strip.innerHTML='';const todayIdx=getTodayIndex();
      DAYS.forEach((name,i)=>{const pill=document.createElement('div');const dayData=state.days[i]||{foco:[],ops:[]};const hasTasks=dayData.foco.length>0||dayData.ops.length>0;pill.className='day-pill'+(i===todayIdx?' today':'')+(i===currentWeekDay?' selected':'')+(hasTasks?' has-tasks':'');pill.innerHTML='<span class="day-label">'+DAY_LETTERS[i]+'</span><div class="day-num">'+getDateLabel(i)+'</div><div class="day-dot"></div>';pill.addEventListener('click',()=>{currentWeekDay=i;renderWeek();});strip.appendChild(pill);});}
    const prog=document.getElementById('week-progress');
    if(prog){prog.innerHTML='<div style="font-size:11px;font-weight:500;color:var(--text-tertiary);letter-spacing:0.04em;margin-bottom:8px;font-family:var(--mono)">progreso semanal</div>';
      DAYS.forEach((name,i)=>{const dayData=state.days[i]||{foco:[],ops:[]};const all=[...dayData.foco,...dayData.ops];const total=all.length,done=all.filter(t=>t.done).length;const pct=total?Math.round((done/total)*100):0;const row=document.createElement('div');row.style.cssText='display:flex;gap:6px;align-items:center;margin-bottom:5px;';row.innerHTML='<div style="font-size:11px;color:var(--text-secondary);width:14px;">'+DAY_LETTERS[i]+'</div><div style="flex:1;height:5px;background:var(--surface);border-radius:3px;overflow:hidden"><div style="width:'+pct+'%;height:100%;background:'+(i===getTodayIndex()?'#534AB7':'#A32D2D')+';border-radius:3px;transition:width 0.4s"></div></div><div style="font-size:10px;font-family:var(--mono);color:var(--text-tertiary);min-width:26px;text-align:right">'+(total?pct+'%':'—')+'</div>';prog.appendChild(row);});}
    const dayLabel=document.getElementById('week-day-label');if(dayLabel)dayLabel.textContent=DAYS[currentWeekDay]+' '+getDateLabel(currentWeekDay);
    const listEl=document.getElementById('week-task-list');
    if(listEl){listEl.innerHTML='';const dayData=state.days[currentWeekDay]||{foco:[],ops:[]};const all=[...dayData.foco.map(t=>({...t,_group:'foco'})),...dayData.ops.map(t=>({...t,_group:'ops'}))];
      ...
      all.forEach(t=>{const row=document.createElement('div');row.className='task'+(t.done?' done':'');const cb=document.createElement('div');cb.className='cb';if(t.done)cb.innerHTML='<svg width="11" height="11" viewBox="0 0 12 12" fill="none"><path d="M2.5 6L5 8.5L9.5 3.5" stroke="white" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"/></svg>';
        ...
        row.appendChild(cb);const txt=document.createElement('div');txt.className='task-content';txt.innerHTML='<div class="task-text">'+t.text+'</div>'+(t.note?'<div class="task-note">'+t.note+'</div>':'');row.appendChild(txt);listEl.appendChild(row);});}
  }
  ...
  function renderNotes(){
    ...
    state.notes.forEach(n=>{const card=document.createElement('div');card.className='note-card';const date=new Date(n.createdAt);const dateStr=date.toLocaleDateString('es',{weekday:'short',day:'numeric',month:'short'})+' · '+date.toLocaleTimeString('es',{hour:'2-digit',minute:'2-digit'});card.innerHTML='<div class="note-title">'+(n.title||'Sin título')+'</div><div class="note-preview">'+(n.body||'').slice(0,120)+((n.body||'').length>120?'…':'')+'</div><div class="note-date">'+dateStr+'</div>';card.addEventListener('click',()=>openNoteModal(n));list.appendChild(card);});
  }
  ...
  function openNoteModal(note){
    ...
    document.getElementById('modal-save').onclick=async()=>{const titleVal=document.getElementById('note-title-input').value.trim();const bodyVal=document.getElementById('note-body-input').value.trim();
    ...
    const doClose=()=>{modal.style.display='none';};document.getElementById('modal-cancel').onclick=doClose;document.getElementById('modal-close').onclick=doClose;
  }
  ...
  function openHitosModal(){
    ...
    const doClose=()=>{modal.style.display='none';render();};document.getElementById('modal-cancel').onclick=doClose;document.getElementById('modal-close').onclick=doClose;
  }

  function renderHitoEditList(){
    const list=document.getElementById('hito-edit-list');if(!list)return;list.innerHTML='';
    state.hitos.forEach((h,idx)=>{const row=document.createElement('div');row.className='hito-edit-row';
      const sel=document.createElement('select');sel.style.cssText='font-size:16px;border:0.5px solid var(--border);border-radius:6px;padding:4px;background:var(--bg);cursor:pointer;';
      ...
      const inp=document.createElement('input');inp.type='text';inp.value=h.text;inp.style.cssText='flex:1;padding:6px 10px;font-size:13px;font-family:var(--font);border:0.5px solid var(--border);border-radius:6px;background:var(--bg);color:var(--text);outline:none;';
      inp.addEventListener('input',()=>{h.text=inp.value;});
      const del=document.createElement('button');del.textContent='🗑️';del.className='ib';del.addEventListener('click',async()=>{await deleteHito(h.id);state.hitos.splice(idx,1);renderHitoEditList();});
  ...
  async function addTask(group){
    ...
    const day=getDayData();day[group].push(task);inp.value='';
    await saveTask(task,group,dayIdx);render();
  }

  function initEvents(){
    document.querySelectorAll('.tab').forEach(btn=>{btn.addEventListener('click',()=>{if(editId)commitEdit();if(editNoteId)commitNoteEdit();document.querySelectorAll('.tab').forEach(b=>b.classList.remove('active'));document.querySelectorAll('.view').forEach(v=>v.classList.remove('active'));btn.classList.add('active');activeTab=btn.dataset.tab;document.getElementById('view-'+activeTab).classList.add('active');render();});});
    ...
    document.getElementById('undo-btn')?.addEventListener('click',async()=>{if(!undoQueue.length)return;const entry=undoQueue.pop();Object.assign(state,entry.state);await loadFromSupabase();clearTimeout(undoTimer);document.getElementById('undo-bar').style.display='none';});
    document.addEventListener('mousedown',e=>{if(editId){const inp=document.getElementById('ei-'+editId);if(inp&&inp.contains(e.target))return;if(e.target.closest('[data-edit]'))return;commitEdit();}if(editNoteId){const inp=document.getElementById('ni-'+editNoteId);if(inp&&inp.contains(e.target))return;if(e.target.closest('[data-addnote]'))return;commitNoteEdit();}});
